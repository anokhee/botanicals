...
function makePetal() {
  ...
  bezierVertex(20, 130, fcp.cp2x, -fcp.cp2y, fcp.cp1x, fcp.cp1y); // if first 2 numbers are changed to 20, 130 it becomes continuous
  endShape();
}
...
function makeFlower() {
  //stroke(baseColor.r, baseColor.g, baseColor.b, 0.5);
  strokeWeight(1);
  ...
  let brightOff =
    Math.random() * (f.brightOff.max - f.brightOff.min) + f.brightOff.min;
  fill(f.r + rOff + brightOff, f.g + gOff + brightOff, f.b + bOff + brightOff);

  let petalCount =
    mainConfig.flowers.petalCount.max -
    mainConfig.flowers.petalCount.min +
    mainConfig.flowers.petalCount.min;
